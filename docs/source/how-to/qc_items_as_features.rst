...
their column count suggests. These four declare ``level = "profile"`` on their
feature classes and are the only QC items usable directly in the profile-level
pipeline; the others must be aggregated per profile via ``agg`` (e.g.
``fail_frac``), as described in :doc:`profile_level_pipeline`. Note that
``qc_pressure_increasing`` is observation-level despite its profile-style
column name: its flag varies within a profile.
