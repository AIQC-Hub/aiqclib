...
Rather than maintain two tables, ``ModelCase`` carries every field as required
(``wrapper_cls``, ``config_name``) or optional (everything else). Tests pick
...
class ModelCase:
    ...
    Optional fields are only consumed by the param-introspection tests in
