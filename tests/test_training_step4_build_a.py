...
- A module-level ``setup_training_step4(test_obj)`` helper (replaced by the
  ``training_input_001_bo002`` fixture in conftest)
...
def _run_test_with_trained_model(ds: BuildModel) -> None:
    """Run ``build_targets`` + ``test_targets`` and assert per-target shapes.

    Used by both the XGBoost-specific test on TestBuildModel and the
    parametrized per-model test below. Previously inlined as
    ``run_test_with_trained_model(test_obj)`` and called from nine separate
    TestCase classes.

    Iterates over TARGETS_NONEMPTY because the NRT_BO_002 config the
    fixtures use excludes pres entirely.
    """
    ds.build_targets()
    ds.test_targets()
...
class TestBuildModel:
    ...
    def test_step_name(self, training_config_001_bo002):
        """step_name == 'build'."""
        ds = BuildModel(training_config_001_bo002)
        assert ds.step_name == "build"

    def test_output_file_names(self, training_config_001_bo002):
        """Default output paths derive from config.path_info; no data-dependence.

        The NRT_BO_002 config uses the same path_info as NRT_BO_001 (both
        reference data_set_1), so the path prefixes match the original
        NRT_BO_001 strings. The dataset_folder_name is also unchanged.
        """
        ds = BuildModel(training_config_001_bo002)
    ...
    def test_training_sets(self, training_config_001_bo002, training_input_001_bo002):
    ...
    def test_shap_flag(self, training_config_001_bo002):
        ...
        training_config_001_bo002.data["step_param_set"]["steps"]["model"]["calculate_shap"] = True
        ds = BuildModel(training_config_001_bo002)
        assert ds.base_model.enable_shap is True

        training_config_001_bo002.data["step_param_set"]["steps"]["model"]["calculate_shap"] = False
        ds = BuildModel(training_config_001_bo002)
        assert ds.base_model.enable_shap is False

    # ----- Default-config XGBoost path -----

    def test_train_with_xgboost(self, training_config_001_bo002, training_input_001_bo002):
    ...
    def test_train_final_model_with_xgboost(self, training_config_001_bo002, training_input_001_bo002):
    ...
    def test_read_models(self, training_config_001_bo002, training_input_001_bo002, training_dir):
        ...
        for tgt in TARGETS_NONEMPTY:
            assert isinstance(ds.test_sets[tgt], pl.DataFrame)
            assert ds.test_sets[tgt].shape == expected_shapes[tgt]

    def test_read_models_no_file(self, training_config_001_bo002, training_input_001_bo002, training_dir):
    ...
    def test_model_objects(self, training_config_001_bo002, training_input_001_bo002):
        """Each target's final_model is a distinct object (not a shared ref).

        With NRT_BO_002 (temp + psal only), there's only one pair to check.
        """
        ds = BuildModel(
            training_config_001_bo002,
            training_sets=training_input_001_bo002.training_sets,
    ...
    def test_test_model_objects(self, training_config_001_bo002, training_input_001_bo002):
    ...
    def test_build_without_training_sets(self, training_config_001_bo002):
        """build_targets with training_sets=None raises ValueError."""
        ds = BuildModel(training_config_001_bo002, training_sets=None, test_sets=None)
        with pytest.raises(ValueError):
            ds.build_targets()

    def test_build_final_model_without_test_sets(self, training_config_001_bo002, training_input_001_bo002):
    ...
    def test_build_final_model_without_training_sets(self, training_config_001_bo002):
        """build_final_model_targets with no inputs raises ValueError."""
        ds = BuildModel(training_config_001_bo002, training_sets=None, test_sets=None)
        with pytest.raises(ValueError):
            ds.build_final_model_targets()

    def test_test_without_model(self, training_config_001_bo002, training_input_001_bo002):
    ...
    def test_write_no_results(self, training_config_001_bo002, training_input_001_bo002):
    ...
    def test_write_empty_contingency_tables(self, training_config_001_bo002, training_input_001_bo002):
    ...
    def test_create_empty_metric_plots(self, training_config_001_bo002, training_input_001_bo002):
    ...
    def test_write_no_models(self, training_config_001_bo002, training_input_001_bo002):
    ...
    def test_write_empty_predictions(self, training_config_001_bo002, training_input_001_bo002):
    ...
    def test_write_reports(self, training_config_001_bo002, training_input_001_bo002, test_output_dir):
        ...
        output_paths = {
            tgt: str(test_output_dir / f"test_test_report_{tgt}.tsv")
            for tgt in TARGETS_NONEMPTY
        }
        for tgt in TARGETS_NONEMPTY:
            ds.output_file_names["report"][tgt] = output_paths[tgt]
    ...
    def test_write_contingency_tables(self, training_config_001_bo002, training_input_001_bo002, test_output_dir):
    ...
    def test_write_shap_values(self, training_config_001_bo002, training_input_001_bo002, test_output_dir):
        ...
        output_paths = {
            tgt: str(test_output_dir / f"test_test_shap_{tgt}.parquet")
            for tgt in TARGETS_NONEMPTY
        }
        for tgt in TARGETS_NONEMPTY:
            ds.output_file_names["shap_value"][tgt] = output_paths[tgt]
    ...
    def test_create_metric_plots(self, training_config_001_bo002, training_input_001_bo002, test_output_dir):
        ...
        output_paths = {
            tgt: str(test_output_dir / f"test_test_metric_plots_{tgt}.svg")
            for tgt in TARGETS_NONEMPTY
        }
        for tgt in TARGETS_NONEMPTY:
            ds.output_file_names["metric_plot"][tgt] = output_paths[tgt]
    ...
    def test_write_models(self, training_config_001_bo002, training_input_001_bo002, test_output_dir):
        ...
        output_paths = {
            tgt: str(test_output_dir / f"test_model_{tgt}.joblib") for tgt in TARGETS_NONEMPTY
        }
        for tgt in TARGETS_NONEMPTY:
            ds.model_file_names[tgt] = output_paths[tgt]
    ...
    def test_write_predictions(self, training_config_001_bo002, training_input_001_bo002, test_output_dir):
        ...
        for tgt in TARGETS_NONEMPTY:
            ds.output_file_names["prediction"][tgt] = output_paths[tgt]
...
class TestModels:
    ...
    def test_base_model(self, case, training_config_001_bo002):
        ...
        """
        training_config_001_bo002.data["step_class_set"]["steps"]["model"] = case.config_name
        ds = BuildModel(training_config_001_bo002)
        assert isinstance(ds.base_model, case.wrapper_cls)

    def test_training(self, case, training_config_001_bo002, training_input_001_bo002):
    ...
    def test_model_output(self, case, training_config_001_bo002, training_input_001_bo002):
    ...
    def test_write_model(self, case, training_config_001_bo002, training_input_001_bo002, test_output_dir):
